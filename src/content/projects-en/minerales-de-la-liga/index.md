...
Based on real data from **LaLiga** players, we designed a system capable of generating crystal representations for all teams in **LaLiga EA Sports** and **LaLiga HyperMotion**. These crystals could be viewed and manipulated in **three dimensions** within the web application, designed with a _mobile-first_ approach.
...
- **Turborepo** as the **monorepo orchestrator**, with 5 independent packages: frontend, backend CMS, design system, video generator, and data manager.
- **Docker** and **Nginx** for containerization and production routing.
...
To handle automatic **AWS RDS password rotation** — managed through **AWS Secrets Manager** — we implemented a self-healing pattern: when the password rotates, Docker's healthcheck detects the failed backend connection and an _autoheal_ container restarts it automatically. On restart, the service fetches the new credential from Secrets Manager without manual intervention.
